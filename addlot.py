...
def main(args):
  ...
  price = 0.1 if color.lower().startswith('various') or args.name else bl.getAveragePrice('PART', args.part_no, color)
  if not args.picture:
    try: 
      image = "HTTP:" + bl.getImage('PART', args.part_no, image_color)['thumbnail_url']
    except KeyError:
      image = "HTTP:" + item['thumbnail_url']
  else:
    image = args.picture
...
if __name__ == "__main__":
  ...
  parser.add_argument("-p", "--picture", action="store", dest="picture")
